...
| `lift-offset` | `0,0.3,0.18` | Application point of lift as a body-frame `x,y,z` offset from the CoG, in scene units (the aircraft spans ~2). Default: high in the wing cell at the quarter-chord |
| `thrust-offset` | `0,0.15,0.45` | Application point of thrust, same format. Default: the propeller boss |
| `drag-offset` | `0,0.1,-0.01` | Application point of drag, same format. Default: the wing-cell trailing edge, above the CoG but below the thrust line |
