...
All four arrows are drawn to a single common scale, so lift and weight tower over thrust and drag — just as they do on a real aeroplane, where thrust is a small fraction of weight. Balanced forces match tip-to-tip: in any steady state, level or not, the arrows sum to zero, and a climb or glide shows up on the gauges and in the velocity — not as surplus lift. When the arrows *don't* balance, the aircraft is accelerating: speed washing off, or the flight path curving.
...
- **The glide** — close the throttle and lower the nose: the aircraft settles into a steady glide with lift still nearly equal to weight, tilted slightly forward, while the along-path component of weight does the job thrust used to do
