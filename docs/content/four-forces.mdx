...
The arrows don't all originate from a single point: their bases sit at the corners of a *moment rectangle* that makes the pitching couples visible. In straight and level flight the lift and weight lines of action are the rectangle's vertical edges and the thrust and drag lines its horizontal edges — lift acts at the wing's quarter-chord, forward of the CoG (a nose-up couple with weight), while the thrust line runs above the drag line (a nose-down couple opposing it). In other flight states the shape deforms into a quadrilateral. The arm sizes are illustrative rather than to scale — unlike the force lengths, which are exact, a true-scale lift/weight arm would be too small to see — and the application points are configurable per aircraft via `lift-offset`, `thrust-offset`, and `drag-offset` (weight always acts at the CoG).
...
- **The couples** — point out where each arrow starts: in level flight the four bases form a rectangle. Lift acts at the wing quarter-chord, forward of the CoG (nose-up couple with weight), and the thrust line runs above the drag line (nose-down couple). In balance they cancel — the reason a trimmed aircraft holds its attitude
...
| `lift-offset` | `0,0.15,0.18` | Application point of lift as a body-frame `x,y,z` offset from the CoG, in scene units (the aircraft spans ~2). Default: the wing quarter-chord, forward of the CoG — the rectangle's top-front corner |
| `thrust-offset` | `0,0.15,0` | A point on the thrust line (the propeller axis), same format. Default: directly above the CoG — the rectangle's top-back corner |
