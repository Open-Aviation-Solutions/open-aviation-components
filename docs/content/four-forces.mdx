...
The arrows also originate where the forces actually act, not from a single point: weight at the centre of gravity, lift at the centre of pressure slightly aft of it, drag at the centre of drag up in the wing cell, and thrust along the propeller axis below that. Lift-behind-weight pitches the nose down; thrust-below-drag pitches it up — the classic opposing couples of the four-forces diagram. The application points are configurable per aircraft via `lift-offset`, `thrust-offset`, and `drag-offset`.
...
- **The couples** — point out where each arrow starts: lift acts behind the CoG (nose-down couple with weight), and the thrust line sits below the centre of drag (nose-up couple). In balance they cancel — the reason a trimmed aircraft holds its attitude
...
| `lift-offset` | `0,0,-0.15` | Application point of lift (the centre of pressure) as a body-frame `x,y,z` offset from the CoG, in scene units (the aircraft spans ~2). The default sits slightly aft of the CoG |
| `thrust-offset` | `0,0.05,0.5` | A point on the thrust line (the propeller axis), same format. The default draws thrust from the propeller, just below the centre of drag |
| `drag-offset` | `0,0.25,0` | The centre of drag, same format. The default sits above the CoG, at the wing cell — a biplane's drag is dominated by wings, struts, and rigging |
