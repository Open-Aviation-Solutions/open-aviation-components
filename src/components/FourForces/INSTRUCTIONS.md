...
- **Arrow objects** — one 3D arrow per force (`_forceArrows`: a solid cylinder shaft plus cone head in the force colour, so the arrows stand apart from the thin grey trace-back lines); `_updateArrows()` repositions and scales them each tick. All four draw on the weight scale, so proportions are true (thrust tops out at ~15% of weight) and balanced forces have equal arrows. The airframe renders translucent (`_applyAirframeOpacity()`, `model-opacity` attribute, default 0.15) so the short thrust/drag arrows aren't hidden inside the fuselage. Each arrow originates where its force physically acts (`_forceOrigins()`: body-frame offsets rotated with the airframe) — lift on the top wing at the quarter-chord, thrust at the propeller boss, drag at the wing trailing edge (configurable via `lift-offset`/`thrust-offset`/`drag-offset`), weight at the CoG (the scene origin, the fixed datum, no offset attribute). A solid grey line (`_updateMomentLines()`) extends each force's line of action back from the arrow base to the farther of its crossings with the perpendicular pair's lines (lift/weight ↔ thrust/drag), so the four lines trace a "moment quadrilateral" — a rectangle in straight and level flight: lift forward of the CoG is a nose-up couple with weight, the thrust line above the drag line a nose-down couple opposing it. A trace-back hides when its force arrow does (e.g. thrust at idle). Display-only: the physics stays a point-mass model with no moment dynamics
